...
//
// Changelog (v12.2):
//    - Added charge-mode time visibility (elapsed / total) for ABSORPTION, FLOAT, IDLE
//    - Serial log now shows NEXT MODE transition (→ IDLE / → BULK)
//    - Globalized internal mode timers for safe debug visibility
//    - Improved simulation support for precise mode-transition testing
//    - Refined ABSORPTION → IDLE behavior for lithium chemistries
//    - Logging structure optimized for readability and field diagnostics
//    - Minor cleanup of timer synchronization and debug consistency
//
// Features:
//    - Full hysteresis-based state machine:
//      BULK → ABSORPTION → FLOAT → BACKUP → IDLE → BULK
//    - Strict separation of:
//        • Charge STATE logic (updateChargeMode)
//        • PWM CONTROL logic (adjustPWM)
//    - All hysteresis, limits, and timings sourced from battery_types.h
//    - Battery-specific CC/CV charging profiles
//    - Enforced minimum dwell time per charge mode
//    - Overvoltage protection forces BACKUP with hard PWM shutdown
//    - Lithium chemistries: NO FLOAT, controlled ABSORPTION → IDLE
//    - IDLE exit requires voltage + time hysteresis
//    - Simulation mode for deterministic state-machine validation
//    - Web interface for monitoring and configuration
//    - ADS1115 + ACS712 battery sensing with auto calibration
//    - Load control (AUTO / MANUAL) with overload & hiccup protection
//    - DS18B20-based thermal fan control
//    - 20x4 LCD + Web UI, WiFi + mDNS, audible alerts
//    - Automatic 24-hour system reboot for long-term stability
//
// Hardware:
...
//    - Buzzer : Active buzzer for audible alerts
//    - Display: 20x4 I2C LCD
//
// Hardware Notes:
//    - INPUT/BAT sensed via shared divider
//      (100k / (17.5k)) ≈ 6.71× scaling
//    - ACS712 calibrated at startup with flash persistence
//    - I2C fixed at 200kHz with GPIO bus recovery
//    - PWM limited to 70–85% for IR2104 bootstrap safety
//    - PWM resolution: 10-bit
//    - IR2104 SD pin (GPIO33):
//        HIGH = Active, LOW = Shutdown (High-Z)
//        Disabled ONLY in IDLE / BACKUP after delay
//
//************************************************************************************
...
//************************************************************************************
// === Charge mode timing visibility for logging ===
//************************************************************************************
unsigned long g_absorptionHoldStart   = 0;
unsigned long g_floatHoldStart        = 0;
unsigned long g_idleVoltageDropStart  = 0;
...
//************************************************************************************
// Compatibility Wrapper: Preserve legacy setChargerPWM() references
//************************************************************************************
void setChargerPWM(uint32_t pwm) {
  writePWMClamped((int)pwm);
}
...
void showWelcomeScreenNonBlocking() {
  ...
  lcd.setCursor(2, 1); lcd.print("Controller v12.1");
...
void readSensors() {
  ...
  ////LiFePO₄ – No Input  simulation
  ////Use this to test BACKUP Mode :
#if SIMULATION_MODE
  input_voltage = 0.0;     // Stable DC supply present
  bat_voltage   = 14.40;    // Near LiFePO4 absorption voltage (14.4V)
  Current_A     = 0.02;     // Very low current (20 mA)
  temperatureC  = 32.0;
  tempValid     = true;
  return;
#endif
  ...
  ////LiFePO₄ – BULK simulation
  ////Use this to test BULK → ABSORPTION:
  //#if SIMULATION_MODE
  //  input_voltage = 24.0;      // Input OK
  //  bat_voltage   = 14.75;     // MUST be >= 14.70 (14.6 + hysteresis)
  //  Current_A     = 1.0;       // High current (still charging)
  //  temperatureC = 32.0;
  //  tempValid    = true;
  //  return;
  //#endif
  ...
  ////LiFePO₄ – ABSORPTION simulation
  ////Use this to test ABS → IDLE logic:
  //
  //#if SIMULATION_MODE
  //  input_voltage = 24.0;
  //  bat_voltage   = 14.75;
  //  Current_A     = 0.02;    // Low current
  //  temperatureC = 32.0;
  //  tempValid    = true;
  //  return;
  //#endif
...
//************************************************************************************
// Charge Mode State Machine - v12.1 FULL HYSTERESIS-BASED CONTROLLER
...
void updateChargeMode() {
  ...
  //************************************************************************************
  // Sync internal timers to globals for logging visibility
  //************************************************************************************
  g_absorptionHoldStart  = absorptionHoldStart;
  g_floatHoldStart       = floatHoldStart;
  g_idleVoltageDropStart = idleVoltageDropStart;
    ...
    case ABSORPTION:
      if (canTransition) {
        if (absorptionHoldStart == 0) {
          ...
          DEBUG_PRINTLN(F("Absorption timer started"));
        }

        bool timerElapsed =
          (now - absorptionHoldStart >= currentBattery.absorption_time_ms);

        bool currentLow =
          (Current_A <= IDLE_CURRENT_THRESHOLD);

        if (timerElapsed || currentLow) {
          mode = IDLE;
          mode_str = "(IDLE)";
          pwm_value = 0;
          pwm_last  = 0;
          ...
          lastModeChange = now;
          absorptionHoldStart = 0;
          DEBUG_PRINTLN(F("ABSORPTION -> IDLE (LiFePO4)"));
        }
      }
      break;
...
//  // Voltages & Current
//  DEBUG_PRINT("Input V: ");
//  DEBUG_PRINT(input_voltage, 2);
//  DEBUG_PRINT(" V  ");
//
//  DEBUG_PRINT("Batt V: ");
//  DEBUG_PRINT(bat_voltage, 2);
//  DEBUG_PRINT(" V  ");
//
//  DEBUG_PRINT("Current: ");
//  DEBUG_PRINT(Current_A, 2);
//  DEBUG_PRINT(" A  ");
//
//  // PWM percentage relative to user-defined limit
//  int dutyPct = 0;
//  if (PWM_MAX_DUTY > 0) {
//    dutyPct = (pwm_value * 100) / PWM_MAX_DUTY;
//    dutyPct = constrain(dutyPct, 0, 100);
//  }
//
//  DEBUG_PRINT("PWM: ");
...
//
//  DEBUG_PRINT("(Limit: ");
//  DEBUG_PRINT(PWM_FULL_SCALE_LIMIT);
//  DEBUG_PRINT("%)  ");
//
//  // Fan PWM
//  int fanDutyPct = (getFanDuty() * 100) / FAN_MAX_DUTY;
//  DEBUG_PRINT("FanPWM: ");
//  DEBUG_PRINT(fanDutyPct);
//  DEBUG_PRINT("%  ");
//
//  // Temperature
//  DEBUG_PRINT("Temp: ");
//  DEBUG_PRINT(temperatureC, 1);
//  DEBUG_PRINT(" °C  ");
//
//  // Mode & Load
//  DEBUG_PRINT("Mode: ");
...
//
//  DEBUG_PRINT("Load: ");
//  DEBUG_PRINT(load_status);
//
//  DEBUG_PRINT(" | LoadMode: ");
//  DEBUG_PRINT(loadControlModeStr);
//
//  // Calibration info
//  DEBUG_PRINT(" | Calib: ");
//  DEBUG_PRINT(ACS_OFFSET_VOLTAGE, 3);
//  DEBUG_PRINTLN(" V");
//}
...
//************************************************************************************
// Serial Logs (Rate-limited to 1 Hz)
// Includes next-mode and timer visibility
//************************************************************************************
void logDataToSerial() {
  static unsigned long last = 0;
  const unsigned long period = 1000;
  ...
  // =====================================================
  // NEXT MODE + TIMER INFO
  // =====================================================
  if (mode == ABSORPTION && g_absorptionHoldStart > 0) {
    unsigned long elapsed = (now - g_absorptionHoldStart) / 1000;
    unsigned long total   = currentBattery.absorption_time_ms / 1000;
  ...
  else if (mode == FLOAT && g_floatHoldStart > 0) {
    unsigned long elapsed = (now - g_floatHoldStart) / 1000;
    unsigned long total   = currentBattery.float_exit_time_ms / 1000;
  ...
  else if (mode == IDLE && g_idleVoltageDropStart > 0) {
    unsigned long elapsed = (now - g_idleVoltageDropStart) / 1000;
    unsigned long total   = currentBattery.idle_exit_time_ms / 1000;
